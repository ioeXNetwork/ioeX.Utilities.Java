    ...
    public void  processMethod() throws Exception{
        ...
        if (jsonArray.size() != 0){
            ...
            if (method.equals("getRawTransaction")){
                System.out.println(IOEXTransaction.genRawTx(param));
            }
        }
        inputStream.close();
...
//                "Eazj14ifau5eH1SP5F8MJRuiSsPMiGbJV1"));
        ...
        outputs.add(new TxOutput("EXkNxfzanRVZumirTudiTBfvMyV2Rt6w95","0.00001", Common.SYSTEM_ASSET_ID,ioeXPrecision));

        RawTx rawTx = ioeX.makeAndSignTx(inputs.toArray(new UTXOTxInput[inputs.size()]),outputs.toArray(new TxOutput[outputs.size()]));
