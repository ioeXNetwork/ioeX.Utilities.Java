...
import org.ioexnetwork.ioex.contract.ContractParameterType;
import org.ioexnetwork.ioex.contract.FunctionCode;
...
import static org.ioexnetwork.api.Verify.Type.*;
import static org.ioexnetwork.common.InterfaceParams.*;
import static org.ioexnetwork.common.Opcode.PACK;
import static org.ioexnetwork.common.Opcode.TAILCALL;
...
public class Basic {
    ...
    public static String genIdentityID(JSONObject jsonObject){
        ...
        String privateKey = jsonObject.getString(PRIVATEKEY);
        String address    = ioeX.getIdentityIDFromPrivate(privateKey);
    ...
    /**
     * Verify that the address is ioex legal
    ...
    public static String genMultiSignAddress(JSONObject jsonObject) {
        ...
        try {
            ...
            address = ioeX.getMultiSignAddress(privateKeyList, M);
        } catch (SDKException e) {
            LOGGER.error(e.toString());
            return e.toString();
        }
    ...
    public static String genNeoContractHashAndAddress(JSONObject jsonObject){
        try {
            ...
            String contractHash = ioeX.genNeoContractHash(contract);
            String contractAddress = ioeX.genNeoContractAddress(contractHash);
            LinkedHashMap<String, Object> resultMap = new LinkedHashMap<String, Object>();
    ...
    public static LinkedList<TxOutput> getOutputs(JSONArray outputs,LinkedList<TxOutput> outputList) throws SDKException {
        for (int t = 0; t < outputs.size(); t++) {
            ...
            outputList.add(new TxOutput(address, amount,Common.SYSTEM_ASSET_ID,ioeXPrecision));
        }
        return outputList;
    }
    ...
    public static LinkedList<PayloadTransferCrossChainAsset> parseCrossChainAsset(JSONArray CrossChainAsset) throws SDKException {
        ...
        for (int n = 0; n < CrossChainAsset.size(); n++) {
            ...
            String address = output.getString(ADDRESS);
            String  amount = output.getString(AMOUNT);
            long longValue = Util.multiplyAmountIOEX(new BigDecimal(amount), ioeXPrecision).longValue();
...
//            }
    ...
    public static PayloadDeploy parsePayloadDeploy(JSONObject json_transaction) throws SDKException {
        ...
        if (PayloadDeploy != null){
            ...
            long longValue = Util.multiplyAmountIOEX(new BigDecimal(gas), ioeXPrecision).toBigInteger().longValue();
