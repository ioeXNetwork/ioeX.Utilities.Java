...
import org.ioexnetwork.common.Curve;
import org.ioexnetwork.common.ErrorCode;
import org.ioexnetwork.common.Helper;
import org.ioexnetwork.common.Scrypt;
...
public class Account {
    ...
    public Account(){
        this.privateKey = ioeX.getPrivateKey();
        this.address = ioeX.getAddressFromPrivate(this.privateKey);
    ...
    public Account(String privateKey){
        ...
        this.address = ioeX.getAddressFromPrivate(privateKey);
        this.scrypt = new Scrypt();
        this.salt = Base64.getEncoder().encodeToString(ECKey.generateKey(16));
    }
    ...
    public static byte[] getGcmDecodedPrivateKey(String encryptedPrivateKey, String passphrase ,String address ,byte[] salt , int n) throws Exception {
        ...
        if (!address.equals(ioeX.getAddressFromPrivate(parsePrivateKey(rawkey)))) {
            throw new Exception(ErrorCode.invalidParam("encryptedPrivateKey address password not match"));
        }
        return rawkey;
    }
